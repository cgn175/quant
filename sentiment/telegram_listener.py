...
class TelegramListener:
    ...
    async def start(self):
        ...
        logger.info("="*60)
        logger.info(f"Monitoring {len(self.channels)} channels: {', '.join(self.channels)}")
    ...
    async def _resolve_channels(self):
        ...
        for channel_username in self.channels:
            try:
                ...
                logger.error(f"  ✗ {channel_username}: Channel is private or doesn't exist")
        ...
        while self.running:
            try:
                ...
                if self.client and self.client.is_connected():
                    ...
                    logger.warning("Keep-alive: client not connected, attempting reconnect...")
                    if self.client:
                        await self.client.connect()
...
async def main():
    ...
    if not settings.telegram_api_id or not settings.telegram_api_hash:
        ...
        logger.error("Set SENTIMENT_TELEGRAM_API_ID and SENTIMENT_TELEGRAM_API_HASH in .env")
        return 1
