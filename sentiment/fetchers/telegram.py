...
class TelegramFetcher(BaseFetcher):
    ...
    async def _exponential_backoff_retry(self, func, *args, **kwargs):
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                wait_time = e.seconds
                print(f"FloodWaitError: Must wait {wait_time} seconds (attempt {attempt + 1}/{MAX_RETRIES})")
                
                # If wait time is too long, give up
                if wait_time > MAX_BACKOFF_SECONDS:
                    print(f"Wait time {wait_time}s exceeds max backoff {MAX_BACKOFF_SECONDS}s, giving up")
                    raise
    ...
    async def _init_client(self):
        ...
        try:
            ...
            await self._exponential_backoff_retry(self.client.connect)
    ...
    async def _fetch_channel_messages(
        ...
        try:
            ...
            print(f"Hit flood limit for channel @{channel_username}, need to wait {e.seconds}s")
        except Exception as e:
            print(f"Error fetching from channel @{channel_username}: {type(e).__name__}: {e}")
    ...
    def _extract_sentiment_score(self, text: str) -> int:
        ...
        positive_words = [
            "surge", "rally", "bull", "gain", "profit", "up", "rise",
            "bullish", "strong", "growth", "pump", "moon", "breakthrough",
            "adoption", "partnership", "launch", "upgrade", "milestone",
        ]
        negative_words = [
            "crash", "dump", "bear", "loss", "down", "fall", "bearish",
            "weak", "decline", "risk", "concern", "drop", "hack", "scam",
            "regulation", "ban", "lawsuit", "fraud",
        ]
