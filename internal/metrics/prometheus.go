...
type Metrics struct {
	...
	LiqFundingRate        prometheus.GaugeVec   // current funding rate by symbol
	LiqOIChangePct        prometheus.GaugeVec   // OI change % (24h) by symbol
	LiqSignalActive       prometheus.GaugeVec   // active signal (0=none, 1=long, 2=short)
	LiqSignalConfidence   prometheus.GaugeVec   // confidence score (0-1) by symbol
	LiqLiquidationCluster prometheus.GaugeVec   // liquidation cluster price by symbol
	LiqSignalsTotal       prometheus.CounterVec // total signals by symbol and direction
}
...
func NewMetrics() *Metrics {
	return &Metrics{
		...
		// Liquidation Strategy metrics
		LiqFundingRate: *promauto.NewGaugeVec(prometheus.GaugeOpts{
			Name: "liq_funding_rate",
			Help: "Current funding rate being monitored for liquidation signals",
		}, []string{"symbol"}),
		LiqOIChangePct: *promauto.NewGaugeVec(prometheus.GaugeOpts{
			Name: "liq_oi_change_pct",
			Help: "Open interest change percentage (24h)",
		}, []string{"symbol"}),
		LiqSignalActive: *promauto.NewGaugeVec(prometheus.GaugeOpts{
			Name: "liq_signal_active",
			Help: "Active liquidation signal (0=none, 1=long_squeeze, 2=short_squeeze)",
		}, []string{"symbol"}),
		LiqSignalConfidence: *promauto.NewGaugeVec(prometheus.GaugeOpts{
			Name: "liq_signal_confidence",
			Help: "Confidence score of liquidation signal (0-1)",
		}, []string{"symbol"}),
		LiqLiquidationCluster: *promauto.NewGaugeVec(prometheus.GaugeOpts{
			Name: "liq_liquidation_cluster",
			Help: "Estimated liquidation cluster price level",
		}, []string{"symbol"}),
		LiqSignalsTotal: *promauto.NewCounterVec(prometheus.CounterOpts{
			Name: "liq_signals_total",
			Help: "Total liquidation signals generated by type",
