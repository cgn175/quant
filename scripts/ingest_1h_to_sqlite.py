#!/usr/bin/env python3
"""Ingest 1H candle data + reuse 4H funding rates into SQLite for ML training.

Creates data/training_1h.db with:
  - candles table: 1H OHLCV data
  - funding table: same 8h funding rates (reused from 4H data)

Funding rates are published every 8 hours regardless of candle timeframe,
so they can be reused as-is.
"""
...
CANDLE_FILES = "data_1h/*_USDT_1h_2190d.parquet"
...
def main():
    parser = argparse.ArgumentParser(description="Ingest 1H parquet data into SQLite")
    parser.add_argument("--db-path", default="data/training_1h.db",
                        help="Path to SQLite DB (default: data/training_1h.db)")
    ...
    print("\nIngesting 1H candle data...")
    ingest_candles(conn, root)

    print("\nIngesting funding data (reusing 4H funding — same 8h rates)...")
